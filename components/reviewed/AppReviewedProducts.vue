    <template>
        <section class="container__main-reviewed">
            ...
            <div class="container__main-reviewed-blocks">
                <div v-if="getReviewed.length === 0" class="container__main-reviewed-blocks-null">
                    <p>Нажаль переглянуті товари відстуні.</p>
                </div>
                <div v-else class="container__main-reviewed-blocks-block" v-for="reviewed in getReviewed"
                    :key="reviewed.id">
                    <div class="container__main-reviewed-blocks-block-img">
                        <nuxt-link :to="`/product/${reviewed.id}`">
                            <img :src="reviewed.images[0]" :alt="reviewed.slug">
                        </nuxt-link>
                    </div>
                    <div class="container__main-reviewed-blocks-block-title">{{ reviewed.title }}</div>
                    <div class="container__main-reviewed-blocks-block-option">
                        <div class="container__main-reviewed-blocks-block-option-price">{{ reviewed.price }} $</div>
...
export default {
    ...
    mounted() {
        this.$store.dispatch('reviewed/getReviewedFromLocal')
    }
}
</script>
